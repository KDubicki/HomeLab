...
- **Network**: single 1GbE NIC. `vmbr0` (`192.168.0.113/24`) plus a private-subnet bridge `vmbr1` (change-log/0004, 2026-07-04): no bridge-ports (no LAN uplink), `10.10.10.0/24`, gatewayed by an LXC `edge` (VMID 101, dual-homed `192.168.0.10`/`10.10.10.1`) running NAT + SSH bastion + DHCP/DNS for the private side.
...
- **One persistently running workload**: `edge` (LXC 101, change-log/0004) — the private-subnet NAT gateway/bastion/DHCP-DNS. Template 9000 remains stopped (a template, not a running guest); no other VMs/containers run continuously yet.
