...
- **Three persistently running workloads**: `edge` (LXC 101, change-log/0004) — the private-subnet NAT gateway/bastion/DHCP-DNS; `vault` (LXC 102, change-log/0005) — a single-node HashiCorp Vault, private-subnet only, TLS + Raft storage on `ssd-data`; `k3s-1` (VM 110, `10.10.10.20`, change-log/0006) — single-node **k3s v1.36.2+k3s1** (bundled Traefik + `ServiceLB`), Helm, the `vault-k8s` Agent Injector, and a sample workload proving Vault-injected secrets + LAN-reachable ingress. Template 9000 remains stopped (a template, not a running guest).
...
- **k3s compute tier live** (change-log/0006, done 2026-07-06): single-node **k3s v1.36.2+k3s1** (VM 110, `10.10.10.20`, private-subnet only), bundled Traefik + `ServiceLB` retained as ingress/LB, Helm installed, provisioned via a new `terraform` `k3s.tf` module invocation and a new Ansible `k3s` role. Vault's **`kubernetes` auth method** is configured (role `k3s-sample` bound to ServiceAccount `sample`/`default`) and the **`vault-k8s` Agent Injector** runs in-cluster (`server.enabled=false`, pointed at Vault's private IP `10.10.10.10` — `vault.lab.internal` is not resolvable from k3s-1, a `terraform/modules/vm` DNS gap carried as a follow-up). `edge`'s (0004) nftables now DNAT LAN `80`/`443` to `k3s-1`'s ingress; kube-API (`6443`) and kubeconfig retrieval stayed bastion/SSH-tunnel-only. Proven end-to-end with a sample Deployment/Service/Ingress: the Vault Agent sidecar rendered a KV secret into the pod (trusting Vault's self-signed cert via a `vault-tls-ca` Kubernetes Secret), the Ingress answered from the LAN through `edge`'s DNAT, and `kubectl get nodes` worked over the tunneled kubeconfig.
